...
export default class Details extends React.Component {
    ...
    componentDidMount() {
        Permissions.askAsync(Permissions.LOCATION)
            ...
            .then(position => {
                ...
                this.getAlerts({ coords: position.coords });
            });
    }
    ...
    getAlerts = ({ zipcode, coords }) =>
        weatherApi('/onecall', { zipcode, coords })
            .then(response => {
                ...
                if (response.cod !== '404') {
                    if (response.alerts !== null) {
                        ...
                        if (this.state.alertReceived !== true && response.alerts[0].description !== 'No location data found') {
                            this.showAlert();
                        } else {
                            this.setState({
                                alertReceived: true,
    ...
    showAlert = () => {
        Alert.alert(
            this.state.alerts[0].event,
            this.state.alerts[0].description,
            [
